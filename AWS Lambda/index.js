...
async function transformJsonToSql(input) {
    ...
    if(additionalData.dataURL) {
     ...
     let sqlValues = mainData.map(item => {
         ...
         let linkedinUrl = item.defaultProfileUrl ? `'${item.defaultProfileUrl.replace(/'/g, "\\'")}'` : 'NULL';
         ...
         let linkedinCompany = item.regularCompanyUrl ? `'${item.regularCompanyUrl.replace(/'/g, "\\'")}'` : 'NULL';
         let companyNameCleaned = item.companyId ? `'${item.companyId.replace(/'/g, "\\'")}'` : 'NULL';
         let containerId = additionalData.Container_ID ? `'${additionalData.Container_ID.replace(/'/g, "\\'")}'` : 'NULL';
         return `(${ICP_TM}, ${domain}, ${industry}, ${fullName}, ${jobTitle}, ${lastName}, ${firstName}, ${sizeRange}, ${companyName}, ${linkedinUrl}, ${companyState}, ${customerName}, ${companyLocation}, ${linkedinCompany}, ${companyNameCleaned}, ${containerId})`;
     }).join(", ");
     let sqlQuery = `INSERT INTO URL_to_Data.URL_to_Data (ICP_TM, domain, industry, full_name, job_title, last_name, first_name, size_range, company_name, linkedin_url, company_state, customer_name, company_location, linkedin_company, company_name_cleaned, Container_ID) VALUES ${sqlValues};`;
     return sqlQuery;
}
